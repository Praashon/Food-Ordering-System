...
const getTopRatedFoods = async (req, res) => {
  try {
    ...
    // First try to get foods with reviews
    let topRatedFoods = await foodModel.find({
      totalReviews: { $gt: 0 }
    })
    .sort({ averageRating: -1, totalReviews: -1 })
    .limit(limit);

    // If no foods with reviews, get all foods (first ones)
    if (topRatedFoods.length === 0) {
      topRatedFoods = await foodModel.find({})
        .limit(limit);
    }

    res.json({ success: true, data: topRatedFoods });
  } catch (error) {
    console.log(error);
...
export { addFood, listFood, removeFood, viewFood, editFood, getTopRatedFoods };
